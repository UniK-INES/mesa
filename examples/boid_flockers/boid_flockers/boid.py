...
class Boid(Agent):
    ...
    def separate(self, nearestNeighbor):
        ...
        """
        self.heading += self.adjustAngle(nearestNeighbor.heading,
                                        self.model.max_separate_angle, away=True)
    ...
    def cohere(self, neighbors):
        ...
        self.heading += self.adjustAngle(self.getAngle(cohere[0], cohere[1]),
                                        self.model.max_cohere_angle, away=False)
    ...
    def align(self, neighbors):
        ...
        if neighbors:
            for neighbor in neighbors:
                ...
                self.model.log.debug(str(self.unique_id) + ": " + str(neighbor.unique_id) +
                              "> " + str(match_vector))
            match_vector /= len(neighbors)
        self.heading += self.adjustAngle(self.getAngle(match_vector[0], match_vector[1]),
                                        self.model.max_align_angle, away=False)
    ...
    def step(self):
        ...
        if nearestNeighbor:
            if distance < self.model.minimum_separation:
                ...
                self.log.debug(str(self.unique_id) + 
                        ": Heading separate: %s > %s", oldHeading, self.heading)
            else:
                oldHeading = self.heading
                self.align(boids)
                self.log.debug(str(self.unique_id) +
                        ": Heading align: %s > %s", oldHeading, self.heading)
                oldHeading = self.heading
                self.cohere(boids)
                self.log.debug(str(self.unique_id) +
                        ": Heading cohere: %s > %s", oldHeading, self.heading)
                
        # normalise heading
        self.setHeading(self.heading)
        distance = math.sqrt(self.headingx**2 + self.headingy**2)
        new_pos = self.pos + np.array([self.headingx,self.headingy])/distance * self.speed
        if not self.model.toroidal:
            if self.model.space.out_of_bounds(new_pos):
                self.headingx *= (-1)
                self.headingy *= (-1)
                self.heading = self.correctAngle(self.heading-180)
                new_pos = self.pos + np.array([self.headingx,self.headingy])\
                    /distance * self.speed
    ...
    def setHeading(self, heading):
        ...
        self.log.debug(str(self.unique_id) + ": " + str(self.heading) + 
                            " - " + str(self.headingy))
